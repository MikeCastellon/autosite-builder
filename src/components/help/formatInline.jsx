...
function renderSegment(segment, key) {
  ...
  if (segment.startsWith('`') && segment.endsWith('`')) {
    return <code key={key} className="px-1 py-0.5 bg-black/5 rounded text-[0.85em]">{segment.slice(1, -1)}</code>;
  }
...
export function formatInline(str) {
  ...
  return paragraphs.map((p, i) => <p key={`p-${i}`} className={i > 0 ? 'mt-2' : ''}>{renderParagraph(p, `p${i}`)}</p>);
}
