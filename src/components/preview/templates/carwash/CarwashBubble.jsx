...
export default function CarwashBubble({ businessInfo, generatedCopy, templateMeta, images = {} }) {
  ...
  const getOrder = buildSectionOrder(copy, ['hero','services','process','about','gallery','testimonials','instagram','cta']);
  ...
  return (
    <div style={{ fontFamily: bodyFont, background: c.bg, color: c.text, minHeight: '100vh', overflowX: 'hidden', margin: 0, padding: 0, containerType: 'inline-size', display: 'flex', flexDirection: 'column' }}>
      ...
      {!hidden('instagram') && copy?.instagramWidgetKey && (
        <section style={{ order: getOrder('instagram'), padding: '80px 5%', background: c.secondary || '#e0f7fa', borderTop: '1px solid #e5e7eb' }}>
