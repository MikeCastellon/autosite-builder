...
export default function MobileRugged({ businessInfo, generatedCopy, templateMeta, images = {} }) {
  ...
  const getOrder = buildSectionOrder(copy, ['hero','statsBar','services','about','gallery','testimonials','instagram','cta']);
  ...
  return (
    <div style={{ fontFamily: font, background: c.bg, color: c.text, minHeight: '100vh', overflowX: 'hidden', margin: 0, padding: 0, containerType: 'inline-size', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* STATS BAR */}
      {!hidden('statsBar') && (
      <section style={{ background: c.secondary || '#232e20', padding: '3.5rem 5%', borderBottom: `1px solid ${c.accent}33` , order: getOrder('statsBar') }}>
      ...
      {/* ABOUT + HOURS */}
      {!hidden('about') && (
      <section id="about" style={{ background: c.secondary || '#232e20', padding: '80px 5%', borderTop: `1px solid ${c.accent}33` , order: getOrder('about') }}>
      ...
      {!hidden('instagram') && copy?.instagramWidgetKey && (
        <section style={{ order: getOrder('instagram'), padding: '80px 5%', background: c.secondary || '#232e20', borderTop: `1px solid ${c.accent}33` }}>
          <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      {/* FOOTER */}
      <footer style={{ background: '#111a0f', padding: '48px 5% 24px', borderTop: `1px solid ${c.accent}33`, order: 9999 }}>
